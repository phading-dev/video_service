...
import { listStorageStartRecordingTasks } from "../db/sql";
...
export class ListStorageStartRecordingTasksHandler extends ListStorageStartRecordingTasksHandlerInterface {
  ...
  public async handle(
    ...
    let rows = await listStorageStartRecordingTasks(
      this.database,
      this.getNow(),
    );
    return {
      tasks: rows.map((row) => ({
        r2Dirname: row.storageStartRecordingTaskPayload.r2Dirname,
